...
public class KnxReq implements IRequester {
	 ...
	 * @throws Exception
	 */
	public String readStatus(String address, String group, String datatype) throws Exception {
	 ...
	 * @throws Exception
	 */
	public void writeStatus(String address, String group, String datatype, String value) throws Exception {
	...
	public CompletableFuture<?> sendRequest(String address, HashMap<String, Object> configuration, Object payload) {
		...
		CompletableFuture<?> output = new CompletableFuture<String>();
		if (requestType.equals("read")) {
			output = CompletableFuture.supplyAsync(() -> {
				try {
					return readStatus(address, configuration.get("group").toString(),
							configuration.get("dataType").toString());
				} catch (Exception e) {
					e.printStackTrace();
				}
            ...
            output = CompletableFuture.runAsync(()->{
				try {
					writeStatus(address, configuration.get("group").toString(), configuration.get("datatype").toString(), payload.toString());
				} catch (Exception e) {
					e.printStackTrace();
				}
			});
        }else if (requestType.equals("subscribe")){
			output = CompletableFuture.runAsync(()->{
				try {
					subscribeToKNXBuffer(address, configuration.get("group").toString(), configuration.get("datatype").toString());
				} catch (Exception e) {
					e.printStackTrace();
				}
			});
